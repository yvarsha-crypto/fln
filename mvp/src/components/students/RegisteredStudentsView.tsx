...
/**
 * Registered Students page — Teachers
 *
 * Per Version_1.0.md §6.6 (List) + §6.7 (Edit) + §6.8 (Deactivate).
 *
 * Features:
 *   - Search (multi-field: name, parent, studentId, aadhaar)
 *   - Sortable columns
 *   - Filter by class (tabs) and status (active/inactive/all)
 *   - Pagination 10 / 25 / 50 / 100
 *   - Edit (slide-over drawer)
 *   - Deactivate / Reactivate (soft lifecycle)
 *   - Student ID displayed (MongoDB ObjectId, 24-char hex)
 *   - Aadhaar displayed masked (XXXXXXXX1234) with reveal toggle (R-6)
 */
export function RegisteredStudentsView({ user, token }: RegisteredStudentsViewProps) {
  ...
  const matched = useMemo(() => {
    ...
    return classStudents.filter((s) => {
      ...
      if ((s.aadharMasked || '').toLowerCase().includes(q)) return true;
    ...
    arr.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'dateOfBirth':
          ...
          break;
        case 'classSection':
          av = getClassSection(a);
          bv = getClassSection(b);
          break;
        case 'parentName':
          av = a.parentName || '';
          bv = b.parentName || '';
  ...
  useEffect(() => {
    setPage(1);
  }, [searchQuery, statusFilter, activeClassId, sortBy, sortDir, pageSize]);
  ...
  function toggleAadhaarReveal(studentId: string) {
    setRevealedAadhaar((prev) => ({ ...prev, [studentId]: !prev[studentId] }));
  }
  ...
  async function handleDeactivateConfirm(student: Student) {
    ...
    try {
      await studentAPI.remove(token, studentId);
      setStudents((prev) =>
        prev.map((s) =>
          (s.studentId || s.id) === studentId ? { ...s, status: 'inactive' } : s
        )
      );
  ...
  async function handleReactivate(student: Student) {
    ...
    const studentId = getStudentId(student);
    try {
      await studentAPI.reactivate(token, studentId);
      setStudents((prev) =>
        prev.map((s) =>
          (s.studentId || s.id) === studentId ? { ...s, status: 'active' } : s
        )
      );
  ...
  function handleSaved(updated: Student) {
    // Merge the saved record back into the list
    setStudents((prev) =>
      prev.map((s) =>
        (s.studentId || s.id) === (updated.studentId || updated.id) ? { ...s, ...updated } : s
      )
    );
    ...
    return (
      <div className="p-8 max-w-md mx-auto bg-white border border-zinc-200 rounded-2xl shadow-sm text-center space-y-4 my-12">
        ...
        <p className="text-sm text-zinc-500">
          You must have at least one registered classroom to view students.
        </p>
        <button
          onClick={() => navigate('/workspace')}
          className="px-4 py-2 bg-zinc-950 text-white font-mono font-medium text-xs rounded-lg hover:bg-zinc-850"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-zinc-200 pb-4 flex flex-col md:flex-row md:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-xs mt-1">
            Search, edit, and manage your class roster. Student IDs are 24-character MongoDB
            ObjectIds; Aadhaar numbers are stored securely and shown masked.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => navigate('/register-student')}
            className="bg-zinc-900 hover:bg-zinc-800 text-white font-mono font-medium text-xs px-4 py-2 rounded-lg inline-flex items-center gap-1.5"
          >
          ...
          <button
            ...
            className="bg-white border border-zinc-200 hover:border-indigo-200 text-zinc-700 font-mono font-medium text-xs px-4 py-2 rounded-lg inline-flex items-center gap-1.5"
          >
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-4 shadow-sm">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex items-center gap-1 text-xs">
            <span className="text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-wider mr-1">
              Status
            </span>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4 text-[11px] font-mono text-zinc-500">
          <span>
            <strong className="text-zinc-900">{counts.total}</strong> total in class
          </span>
          ...
          {(searchQuery || statusFilter !== 'active') && (
            <span>
              <strong className="text-zinc-900">{sorted.length}</strong> after filter
            </span>
          )}
        </div>
      </div>
            ...
            <thead>
              <tr className="text-left border-b border-zinc-200 bg-zinc-50/50 text-zinc-500">
                ...
                <SortableHeader
                  ...
                  className="w-32"
                />
                <th className="px-3 py-2 font-mono font-bold uppercase text-[10px] tracking-wider w-48">
                  Aadhaar
                </th>
                ...
                <SortableHeader
                  ...
                  className="w-28"
                />
                <th className="px-3 py-2 font-mono font-bold uppercase text-[10px] tracking-wider text-right">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                paginated.map((s) => {
                  ...
                  if (aadhaar && !aadhaarRevealed) {
                    if (/^\d{12}$/.test(aadhaar)) {
                      aadhaarDisplay = 'X'.repeat(8) + aadhaar.slice(-4);
                    } else if (aadhaar.length > 4) {
                      aadhaarDisplay = 'X'.repeat(aadhaar.length - 4) + aadhaar.slice(-4);
                    }
                  }
                  const status = s.status || 'active';
                  return (
                    <tr
                      key={sid || Math.random()}
                      className="border-b border-zinc-100 last:border-b-0 hover:bg-slate-50/50"
                    >
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1">
                          <span
                            className="font-mono text-[11px] text-zinc-900 select-all truncate max-w-[10rem]"
                            title={sid}
                          >
                      ...
                      <td className="px-3 py-2 font-mono text-zinc-700 whitespace-nowrap">
                        {s.dateOfBirth || (s.age != null ? `~${s.age}y` : '—')}
                      </td>
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          {aadhaar && aadhaar.length > 4 && (
                            <button
                              onClick={() => toggleAadhaarReveal(sid)}
                              className="text-zinc-400 hover:text-slate-700"
                              aria-label={aadhaarRevealed ? 'Hide Aadhaar' : 'Reveal Aadhaar'}
                              aria-pressed={aadhaarRevealed}
                              title={aadhaarRevealed ? 'Hide unmasked Aadhaar' : 'Reveal (local) Aadhaar'}
                            >
                              {aadhaarRevealed ? (
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="text-zinc-500 hover:text-indigo-600 p-1"
                            aria-label={`Edit ${getStudentName(s)}`}
                            title="Edit"
                          >
                          ...
                          {status === 'active' ? (
                            <button
                              onClick={() => setConfirmingDeactivate(s)}
                              disabled={actionBusy}
                              className="text-zinc-500 hover:text-rose-600 p-1 disabled:opacity-50"
                              aria-label={`Deactivate ${getStudentName(s)}`}
                              title="Deactivate"
                            >
                            ...
                            <button
                              ...
                              disabled={actionBusy}
                              className="text-zinc-500 hover:text-emerald-600 p-1 disabled:opacity-50"
                              aria-label={`Reactivate ${getStudentName(s)}`}
                              title="Reactivate"
                            >
        ...
        {paginated.length > 0 && (
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-3 py-3 border-t border-zinc-200 bg-zinc-50/30">
            <div className="text-[11px] font-mono text-zinc-500">
              ...
              of <strong className="text-zinc-900">{sorted.length}</strong>
            </div>
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-[10px] font-mono text-zinc-500 mx-2">
              ...
              <div className="inline-flex border border-zinc-200 rounded-lg overflow-hidden">
                <button
                  ...
                  disabled={safePage === 1}
                  className="px-2 py-1 bg-white text-zinc-700 hover:bg-zinc-50 disabled:opacity-50 border-r border-zinc-200"
                ...
                <button
                  ...
                  className="px-2 py-1 bg-white text-zinc-700 hover:bg-zinc-50 disabled:opacity-50"
        ...
        description={
          confirmingDeactivate ? (
            <div className="space-y-2">
              <p>
                Deactivating <strong>{getStudentName(confirmingDeactivate)}</strong> will hide them
                from the active roster. You can reactivate them at any time.
              </p>
      ...
      {toast && (
        <div
          className="fixed bottom-6 right-6 z-[10000] flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-xs font-bold text-white shadow-2xl border border-slate-800"
          role="status"
          aria-live="polite"
        >
...
// =============================================================
// Sub-components (kept local to this page to avoid growing the
// global surface area for these one-off concerns)
// =============================================================
...
function SortableHeader({ label, sortKey, current, dir, onClick, className = '' }: SortableHeaderProps) {
...
function StatusBadge({ status }: { status: 'active' | 'inactive' | string }) {
  if (status === 'active') {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
        ● Active
      </span>
    );
  }
  if (status === 'inactive') {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider bg-rose-50 text-rose-700 border border-rose-200">
        ○ Inactive
      </span>
    );
  }
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-wider bg-slate-100 text-slate-600 border border-slate-200">
      {status}
    </span>
  );
}