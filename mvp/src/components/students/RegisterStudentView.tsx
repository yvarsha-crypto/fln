...
export function RegisterStudentView({ user, token }: RegisterStudentViewProps) {
  ...
  // Active (live) validation. Updates errors as the user types — once a field
  // has been blurred or the user has attempted to submit.
  useEffect(() => {
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (err?.response?.data?.error) {
        ...
        setError('v2 API unavailable (MongoDB not connected). Please try the standard backend.');
      } else {
    ...
    <div className="space-y-6">
      ...
      {createdStudent && (
        <SuccessBanner
          ...
          onViewMyStudents={() => navigate('/registered-students')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          {/* Class & Section — auto-filled, displayed in a read-only block */}
          ...
          {/* Hidden school code (server-derived, never user-edited) */}
        ...
        {/* === Sidebar (geography confirmation + quick actions) === */}
