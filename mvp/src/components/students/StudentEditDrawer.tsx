...
/**
 * Slide-over drawer for editing an existing student's registration fields.
 *
 * Editable fields (Version_1.0.md §6.7):
 *   - studentName, dateOfBirth, gender, aadhaarNumber, parentName
 * Immutable (read-only, displayed for context):
 *   - studentId (the MongoDB ObjectId), class, section, schoolId/schoolCode,
 *     createdBy, createdAt
 *
 * The drawer pattern matches the existing project style:
 *   `bg-white border-l border-zinc-200 shadow-2xl` on a `bg-black/30 backdrop-blur-sm` overlay.
 */
export const StudentEditDrawer: React.FC<StudentEditDrawerProps> = ({
  ...
  // Sync form state when the student prop changes (open / switch student)
  useEffect(() => {
    if (!student) return;
    setForm({
  ...
  const classNameDisplay =
    student.classGroup || (student.class ? `Class ${student.class}` : '—');
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (err?.response?.data?.error) {
        ...
        setServerError('v2 API unavailable (MongoDB not connected). Please try the standard backend.');
      } else {
        setServerError(err?.message || 'Network error. Check connection settings.');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div
      className="fixed inset-0 z-[9990] flex justify-end bg-black/30 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="w-full sm:max-w-lg bg-white border-l border-zinc-200 shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-zinc-200 flex items-start justify-between gap-3">
          <div>
            <h2
              ...
              className="text-base font-display font-semibold text-zinc-900 flex items-center gap-2"
            >
              <UserCog className="h-4 w-4 text-indigo-600" />
              Edit Student
            </h2>
            <p className="text-xs text-zinc-500 mt-0.5">
              <span className="font-mono">{form.studentName || '—'}</span>
            </p>
          </div>
          <button
            onClick={onClose}
            disabled={submitting}
            className="text-zinc-400 hover:text-zinc-700 p-1 rounded disabled:opacity-50"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        >
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <div className="font-mono text-zinc-900 select-all break-all">
                  {studentIdDisplay}
                </div>
              </div>
              <div>
          ...
          {/* Editable fields */}
          <div>
            <h3 className="text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-wider mb-3 border-b border-zinc-100 pb-1">
              Editable fields
            </h3>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <RadioGroup
                  ...
                  onChange={(v) => setForm((s) => ({ ...s, gender: v as Gender }))}
                  error={errors.gender}
                  required
                />
              </div>
        ...
        <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2 bg-zinc-50/50">
          ...
          <button
            ...
            disabled={submitting}
            className="px-4 py-2 rounded-lg bg-zinc-900 text-white text-xs font-mono font-semibold hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-1.5"
          >
            {submitting ? (
